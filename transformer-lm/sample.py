"""Generate text from a trained Transformer checkpoint.
...
def load(ckpt_path, device):
    ...
    model = CharTransformerLM(len(vocab), d_model=cfg["d_model"],
                              n_layers=cfg["n_layers"], n_heads=cfg["n_heads"],
                              d_ff=cfg["d_ff"], max_len=cfg["seq_len"],
...
def nll(model, vocab, text, device):
    ...
    ids = torch.tensor(vocab.encode(text).astype("int64"),
                       device=device).unsqueeze(0)[:, :model.max_len + 1]
    logits = model(ids[:, :-1])
    return torch.nn.functional.cross_entropy(
        logits.reshape(-1, logits.size(-1)), ids[:, 1:].reshape(-1)).item()
...
def main(args):
    ...
    if args.score:
        print(f"\nNLL: {nll(model, vocab, args.score, device):.4f}",
              file=sys.stderr)
